...
FINAL="$DIR/transaction_control.mp4"
...
cp "$FINAL" "$SITE_DIR/tut_02_transaction_control.mp4"

# Grab a thumbnail from the SAVEPOINT scene.
ffmpeg -y -ss 00:01:05 -i "$FINAL" -frames:v 1 -vf "scale=480:-1" \
  "$SITE_DIR/thumbs/tut_02_transaction_control.png"
