...
SCENES=(
  ...
  S02_WhatExistsMeans
  S03_NeedForAll
  S04_ImplicationSetup
  S05_ImplicationTruthTable
  S06_DoubleNotExists
  S07_Walkthrough
  S08_Takeaway
)
...
FINAL="$DIR/exists_implication.mp4"
...
cp "$FINAL" "$SITE_DIR/tut_04_exists_implication.mp4"

# Grab a thumbnail from the double-NOT-EXISTS scene.
ffmpeg -y -ss 00:01:13 -i "$FINAL" -frames:v 1 -vf "scale=480:-1" \
  "$SITE_DIR/thumbs/tut_04_exists_implication.png"
