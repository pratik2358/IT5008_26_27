...
\begin{tikzpicture}[scale=0.9, every node/.style={transform shape}]
  ...
  \node[relationship] (r3) at (7,-2.5) {loan};
  \draw[eredge] (r2) -- (r3);
  \node at (10.3,-2.5) [font=\scriptsize, align=left] {a relationship can connect\\straight to \emph{another}\\relationship's diamond ---\\that participant is an\\\textbf{aggregate} (e.g.\ \textbf{own}\\inside \textbf{loan})};
\end{tikzpicture}
\end{center}
\vspace{0.3em}
\small
...
\begin{tikzpicture}
  ...
  \draw[eredge, visible on=<12->] (student) -- node[above,sloped,pos=0.28]{\scriptsize\itshape owner} (own);
  ...
  \draw[eredge, visible on=<14->] (student) -- node[above,sloped,pos=0.28]{\scriptsize\itshape borrower} (loan);
  \draw[eredge, visible on=<14->] (loan) -- (own);
  ...
  \draw[eredge, visible on=<16->] (been) -- (loan);
  ...
  \cardlabel{(-2.3,6.55)}{(0,n)}{23}{24}   % 7. loan -- Students (borrower)
  \cardlabel{(2.3,6.55)}{(0,n)}{24}{25}    % 8. own -- Students (owner)
...
\begin{itemize}
  ...
  \item \textbf{Two} relationships act as \textbf{aggregates}: their diamond is connected to \emph{directly}, by another relationship, rather than only to entities:
  \begin{itemize}
    \item \textbf{loan} connects straight to the \textbf{own} diamond, because it needs ``this particular owned copy'' as a whole, not just \texttt{Students} or \texttt{Copy} alone.
    \item \textbf{been} connects straight to the \textbf{loan} diamond, because it needs ``this particular loan event'' as a whole, not just \texttt{Students}, \textbf{own}, or \texttt{Borrow} alone.
  \end{itemize}
